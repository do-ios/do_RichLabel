...
#import "DODTScriptExpression.h"
#import "DODTScriptVariable.h"
#import "NSScanner+DTScripting.h"
#import "DODTLog.h"


@implementation DODTScriptExpression
{
	NSArray *_parameters;
	NSArray *_parameterNames;
	DODTScriptVariable *_receiver;
	SEL _selector;
}

+ (DODTScriptExpression *)scriptExpressionWithString:(NSString *)string
{
	return [[DODTScriptExpression alloc] initWithString:string];
}
...
- (NSString *)description
{
	...
	{
		[self enumerateParametersWithBlock:^(NSString *name, DODTScriptVariable *variable, BOOL *stop) {
...
- (BOOL)_parseString:(NSString *)string
{
	...
	DODTScriptVariable *receiver = nil;
	...
	if ([decider isEqualToString:@":"])
	{
		...
		DODTScriptVariable *parameter = nil;
		...
		else
		{
			while (![scanner isAtEnd] && ![scanner scanString:@"]" intoString:nil])
			{
				if ([scanner scanCharactersFromSet:[NSCharacterSet alphanumericCharacterSet] intoString:&parameterName])
				{
					...
					DODTScriptVariable *variable = nil;
...
- (void)enumerateParametersWithBlock:(DTScriptExpressionParameterEnumerationBlock)block
{
	for (NSUInteger i=0; i<[_parameters count]; i++)
	{
		...
		DODTScriptVariable *variable = [_parameters objectAtIndex:i];
