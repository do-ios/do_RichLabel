...
static DODTAsyncFileDeleter *_sharedInstance;


// private utilites
@interface DODTAsyncFileDeleter ()
- (BOOL)_supportsTaskCompletion;
@end


@implementation DODTAsyncFileDeleter

+ (DODTAsyncFileDeleter *)sharedInstance
{
	static dispatch_once_t instanceOnceToken;
	dispatch_once(&instanceOnceToken, ^{
		_sharedInstance = [[DODTAsyncFileDeleter alloc] init];
	});
