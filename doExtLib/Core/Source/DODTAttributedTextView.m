...
@interface DODTAttributedTextView ()
...
@implementation DODTAttributedTextView
{
...
{
	return [DODTAttributedTextContentView class];
}
...
- (void)scrollRangeToVisible:(NSRange)range animated:(BOOL)animated
{
	...
	DODTCoreTextLayoutLine *line = [self.attributedTextContentView.layoutFrame lineContainingIndex:range.location];
...
- (DODTAttributedTextContentView *)attributedTextContentView
{
	if (!_attributedTextContentView)
	{
		...
		if ([classToUse isSubclassOfClass:[DODTAttributedTextContentView class]])
		{
			Class layerClass = [DODTAttributedTextContentView layerClass];
			
			if (![layerClass isSubclassOfClass:[CATiledLayer class]])
			{
				[DODTAttributedTextContentView setLayerClass:[DODTTiledLayerWithoutFade class]];
		...
		{
			[DODTAttributedTextContentView setLayerClass:previousLayerClass];
		}
...
- (void)setBackgroundColor:(DODTColor *)newColor
{
	if ([newColor alphaComponent]<1.0)
	{
		...
		_attributedTextContentView.backgroundColor = [DODTColor clearColor];
		self.opaque = NO;
	}
	else 
	{
...
- (UIView *)backgroundView
{
	if (!_backgroundView)
	{
		...
		_backgroundView.backgroundColor	= [DODTColor whiteColor];
...
- (void)setBackgroundView:(UIView *)backgroundView
{
	if (_backgroundView != backgroundView)
	{
		...
		{
			_attributedTextContentView.backgroundColor = [DODTColor whiteColor];
			_attributedTextContentView.opaque = YES;
		}
	}
}
