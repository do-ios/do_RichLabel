...
#import "DODTCoreTextLayoutFrameAccessibilityElementGenerator.h"
#import "DODTBlockFunctions.h"
...
@interface DODTAttributedTextContentView () <DTAccessibilityViewProxyDelegate>
{
	...
	DODTCoreTextLayouter *_layouter;
...
@implementation DODTAttributedTextContentView (Tiling)
...
@implementation DODTAttributedTextContentView (Cursor)
...
@implementation DODTAttributedTextContentView
...
- (void)setup
{
	...
	{
		self.backgroundColor = [DODTColor whiteColor];
	}
...
- (void)layoutSubviewsInRect:(CGRect)rect
{
	...
	DODTCoreTextLayoutFrame *theLayoutFrame = self.layoutFrame;
	...
	for (DODTCoreTextLayoutLine *oneLine in lines)
	{
		...
		for (DODTCoreTextGlyphRun *oneRun in oneLine.glyphRuns)
		{
			...
			if (runRange.location>=skipRunsBeforeLocation)
			{
				...
				DODTTextAttachment *attachment = oneRun.attributes[NSAttachmentAttributeName];
				...
				if (linkURL && !attachment)
				{
					...
					{
						DODTCoreTextGlyphRun *followingRun = oneLine.glyphRuns[i];
				...
				if (attachment)
				{
					...
					else
					{
						...
						if ([attachment isKindOfClass:[DODTDictationPlaceholderTextAttachment class]])
						{
							newCustomAttachmentView = [DODTDictationPlaceholderView placeholderView];
...
- (void)drawLayer:(CALayer *)layer inContext:(CGContextRef)ctx
{
	...
	DODTCoreTextLayoutFrame *theLayoutFrame = self.layoutFrame; // this is synchronized
...
- (CGSize)suggestedFrameSizeToFitEntireStringConstraintedToWidth:(CGFloat)width
{
	...
	DODTCoreTextLayoutFrame *tmpLayoutFrame = [self.layouter layoutFrameWithRect:rect range:NSMakeRange(0, 0)];
...
- (void)setBackgroundColor:(DODTColor *)newColor
{
...
- (DODTCoreTextLayouter *)layouter
{
	@synchronized(self)
	{
		if (!_layouter)
		{
			if (_attributedString)
			{
				_layouter = [[DODTCoreTextLayouter alloc] initWithAttributedString:_attributedString];
...
- (void)setLayouter:(DODTCoreTextLayouter *)layouter
{
	@synchronized(self)
	{
...
- (DODTCoreTextLayoutFrame *)layoutFrame
{
	@synchronized(self)
	{
		DODTCoreTextLayouter *theLayouter = self.layouter;
		...
		if (!_layoutFrame)
		{
			...
			if (theLayouter)
			{
				...
				{
					DT_WEAK_VARIABLE DODTAttributedTextContentView *weakself = self;
					
					[_layoutFrame setTextBlockHandler:^(DODTTextBlock *textBlock, CGRect frame, CGContextRef context, BOOL *shouldDrawDefaultBackground) {
						
						DODTAttributedTextContentView *strongself = weakself;
...
- (void)setLayoutFrame:(DODTCoreTextLayoutFrame *)layoutFrame
{
	@synchronized(self)
	{
...
- (NSArray *)accessibilityElements
{
	if (!_accessibilityElements)
	{
		...
		_accessibilityElements = [generator accessibilityElementsForLayoutFrame:self.layoutFrame view:self attachmentViewProvider:^id(DODTTextAttachment *attachment) {
			...
			return [[DODTAccessibilityViewProxy alloc] initWithTextAttachment:attachment delegate:self];
		}];
	}
...
- (UIView *)viewForTextAttachment:(DODTTextAttachment *)textAttachment proxy:(DODTAccessibilityViewProxy *)proxy
{
...
@implementation DODTAttributedTextContentView (Drawing)
