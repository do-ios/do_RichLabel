...
{
	if ([(__bridge id)context isKindOfClass:[DODTTextAttachment class]])
	{
		return [(__bridge DODTTextAttachment *)context ascentForLayout];
	}
	return 0;
}
CGFloat embeddedObjectGetDescentCallback(void *context)
{
	if ([(__bridge id)context isKindOfClass:[DODTTextAttachment class]])
	{
		return [(__bridge DODTTextAttachment *)context descentForLayout];
	}
	return 0;
}
...
CGFloat embeddedObjectGetWidthCallback(void * context)
{
	if ([(__bridge id)context isKindOfClass:[DODTTextAttachment class]])
	{
		return [(__bridge DODTTextAttachment *)context displaySize].width;
	}
	return 35;
}
