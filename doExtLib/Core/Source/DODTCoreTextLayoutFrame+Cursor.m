...
@implementation DODTCoreTextLayoutFrame (Cursor)
...
- (NSInteger)closestCursorIndexToPoint:(CGPoint)point
{
	...
	DODTCoreTextLayoutLine *firstLine = [lines objectAtIndex:0];
	if (point.y < CGRectGetMinY(firstLine.frame))
	{
		return 0;
	}
	
	DODTCoreTextLayoutLine *lastLine = [lines lastObject];
	...
	// find closest line
	DODTCoreTextLayoutLine *closestLine = nil;
...
{
	DODTCoreTextLayoutLine *line = [self lineContainingIndex:index];
