...
@interface DODTCoreTextFontCollection ()
...
static DODTCoreTextFontCollection *_availableFontsCollection = nil;


@implementation DODTCoreTextFontCollection
{
	NSArray *_fontDescriptors;
	NSCache *_fontMatchCache;
}

+ (DODTCoreTextFontCollection *)availableFontsCollection
{
	static dispatch_once_t predicate;

	dispatch_once(&predicate, ^{
		_availableFontsCollection = [[DODTCoreTextFontCollection alloc] initWithAvailableFonts];
	});
...
- (DODTCoreTextFontDescriptor *)matchingFontDescriptorForFontDescriptor:(DODTCoreTextFontDescriptor *)descriptor
{
	DODTCoreTextFontDescriptor *firstMatch = nil;
	...
	if (firstMatch)
	{
		DODTCoreTextFontDescriptor *retMatch = [firstMatch copy];
	...
	if (!_fontDescriptors)
	{
		...
		if (matchingFonts)
		{
			...
			for (NSInteger i=0; i<CFArrayGetCount(matchingFonts); i++)
			{
				...
				DODTCoreTextFontDescriptor *desc = [[DODTCoreTextFontDescriptor alloc] initWithCTFontDescriptor:fontDesc];
...
- (NSArray *)fontFamilyNames
{
	...
	for (DODTCoreTextFontDescriptor *oneDescriptor in [self fontDescriptors])
	{
