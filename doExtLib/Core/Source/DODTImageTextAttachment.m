...
#if TARGET_OS_IPHONE
	#import "DODTAnimatedGIF.h"
#endif

static NSString *dataPath = @"data://";
static NSString *sourcePath = @"source://";
static NSString *initPath = @"initdata://";

static NSCache *imageCache = nil;

@interface DODTImageTextAttachment () // private stuff
...
@implementation DODTImageTextAttachment
{
	DODTImage *_image;
}
...
- (id)initWithImage:(DODTImage *)image
{
	self = [super init];
...
- (void)_decodeImageFromElement:(DODTHTMLElement *)element options:(NSDictionary *)options
{
	// get base URL
	...
	{
		if ([src hasPrefix:@"data:"] && ![src hasPrefix:dataPath])
		{
			...
			if (!decodedData)
			{
				...
				if (range.length)
				{
					...
					decodedData = [DODTBase64Coding dataByDecodingString:encodedData];
				}
			}
			...
			if (decodedData)
			{
				DODTImage *decodedImage = [[DODTImage alloc] initWithData:decodedData];
					...
					if ([widthStr hasSuffix:@"px"] && [heightStr hasSuffix:@"px"])
					{
							 ...
							 sizeAccordingToStyle.height != 0 && sizeAccordingToStyle.height < decodedImage.size.height)
						{
							...
							if (scale>=2.0 && scale<=5.0)
							{
								...
								decodedImage = [DODTImage imageWithCGImage:decodedImage.CGImage scale:scale orientation:decodedImage.imageOrientation];
#else
			...
			if (![contentURL scheme])
			{
				...
				else
				{
                    ...
                    else
                    {
                        ...
                        bundle = [NSBundle bundleForClass:[DODTTextAttachment class]];
                ...
                if([src hasPrefix:dataPath] || [src hasPrefix:sourcePath] || [src hasPrefix:initPath])
                {
                    NSString * path = [doIOHelper GetLocalFileFullPath:_do_extern_RichLabelModel.CurrentPage.CurrentApp :src];
                    if (path)
                    {
                        contentURL = [NSURL fileURLWithPath:path];
                    }
	...
	{
		DODTImage *image = _image;
		
		// let's check if we have a cached image already then we can inspect that
		if (!_image)
		{
			image = [[DODTImageTextAttachment sharedImageCache] objectForKey:[contentURL absoluteString]];
		}
		...
		if (!image)
		{
...
#endif
				{
					image = [[DODTImage alloc] initWithContentsOfFile:[contentURL path]];
				}
			}
			...
			if (image)
			{
				[[DODTImageTextAttachment sharedImageCache] setObject:image forKey:[contentURL absoluteString]];
			}
		}
...
- (void)_updateSizesFromImage:(DODTImage *)image
{
...
- (NSString *)dataURLRepresentation
{
	...
	NSString *encoded = [DODTBase64Coding stringByEncodingData:data];
...
- (DODTImage *)image
{
	if (!_image)
	{
		if (_contentURL)
		{
			DODTImage *image = [[DODTImageTextAttachment sharedImageCache] objectForKey:[_contentURL absoluteString]];
			
			// only local files can be loaded into cache
			if (!image && [_contentURL isFileURL])
			{
				image = [[DODTImage alloc] initWithContentsOfFile:[_contentURL path]];
				
				// cache it
				if (image)
				{
					[[DODTImageTextAttachment sharedImageCache] setObject:image forKey:[_contentURL absoluteString]];
				}
			}
