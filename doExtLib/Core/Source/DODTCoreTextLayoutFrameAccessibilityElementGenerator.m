...
- (NSArray *)accessibilityElementsForLayoutFrame:(DODTCoreTextLayoutFrame *)frame view:(UIView *)view attachmentViewProvider:(DTAttachmentViewProvider)block
{
...
- (NSArray *)accessibilityElementsInParagraphAtIndex:(NSUInteger)index layoutFrame:(DODTCoreTextLayoutFrame *)frame view:(UIView *)view attachmentViewProvider:(DTAttachmentViewProvider)block
{
	NSMutableArray *elements = [NSMutableArray array];
...
- (void)enumerateAccessibleGroupsInFrame:(DODTCoreTextLayoutFrame *)frame forParagraphAtIndex:(NSUInteger)index usingBlock:(void(^)(NSDictionary *attrs, NSRange substringRange, BOOL *stop, NSArray *runs))block
{
	...
	[frame.attributedStringFragment enumerateAttributesInRange:paragraphRange options:0 usingBlock:^(NSDictionary *attrs, NSRange range, BOOL *stop) {
		...
		for (DODTCoreTextLayoutLine *line in lines)
		{
...
{
	DODTTextAttachment *attachment = [attributes objectForKey:NSAttachmentAttributeName];
...
- (DODTAccessibilityElement *)accessibilityElementForTextInAttributedString:(NSAttributedString *)attributedString atRange:(NSRange)range attributes:(NSDictionary *)attributes run:(NSArray *)runs view:(UIView *)view
{
	NSString *text = [attributedString.string substringWithRange:range];
	
	DODTAccessibilityElement *element = [[DODTAccessibilityElement alloc] initWithParentView:view];
	...
	if (runs.count > 1)
	{
		DODTCoreTextGlyphRun *run = [runs objectAtIndex:0];
...
- (UIView *)viewForAttachment:(DODTTextAttachment *)attachment attachmentViewProvider:(DTAttachmentViewProvider)block
{
	UIView *view = nil;
...
- (CGRect)frameForRuns:(NSArray *)runs
{
	...
	for (DODTCoreTextGlyphRun *run in runs)
