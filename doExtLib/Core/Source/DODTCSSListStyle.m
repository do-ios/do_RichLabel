...
@implementation DODTCSSListStyle
{
	BOOL _inherit;
...
{
	DTCSSListStyleType type = [DODTCSSListStyle listStyleTypeFromString:string];
...
{
	DTCSSListStylePosition position = [DODTCSSListStyle listStylePositionFromString:string];
 ...
 */
- (BOOL)isEqualToListStyle:(DODTCSSListStyle *)otherListStyle
{
	if (!otherListStyle)
	{
		return NO;
	...
	if (![otherListStyle isKindOfClass:[DODTCSSListStyle class]])
	{
		return NO;
	}
...
{
	DODTCSSListStyle *newStyle = [[DODTCSSListStyle allocWithZone:zone] init];
