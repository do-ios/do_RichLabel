...
@interface DODTCoreTextFontDescriptor ()
...
@implementation DODTCoreTextFontDescriptor
{
...
+ (void)initialize
{
	...
	if (self != [DODTCoreTextFontDescriptor class])
	{
		return;
	}
	...
	for (NSDictionary *oneOverride in fileArray)
	{
		...
		if (smallcaps)
		{
			[DODTCoreTextFontDescriptor setSmallCapsFontName:overrideFontName forFontFamily:fontFamily bold:bold italic:italic];
		}
		else
		{
			[DODTCoreTextFontDescriptor setOverrideFontName:overrideFontName forFontFamily:fontFamily bold:bold italic:italic];
		}
	}
...
+ (void)_createDictionaryOfAllAvailableFontOverrideNamesWithCompletion:(void(^)(NSDictionary *dictionary))completion
{
	...
	dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
		...
		DODTCoreTextFontCollection *allFonts = [DODTCoreTextFontCollection availableFontsCollection];
		...
		for (DODTCoreTextFontDescriptor *oneFontDescriptor in sortedFonts)
		{
...
+ (DODTCoreTextFontDescriptor *)fontDescriptorWithFontAttributes:(NSDictionary *)attributes
{
	return [[DODTCoreTextFontDescriptor alloc] initWithFontAttributes:attributes];
}

+ (DODTCoreTextFontDescriptor *)fontDescriptorForCTFont:(CTFontRef)ctFont
{
	return [[DODTCoreTextFontDescriptor alloc] initWithCTFont:ctFont];
}
...
{
	if ([DODTCoreTextFontDescriptor smallCapsFontNameforFontFamily:_fontFamily bold:self.boldTrait italic:self.italicTrait])
	{
		return YES;
	}
...
- (CTFontRef)_findOrMakeMatchingFont
{
	...
	if (_fontFamily && !_fontName) // forced font name takes precedence
	{
		if (_smallCapsFeature)
		{
			overrideName = [DODTCoreTextFontDescriptor smallCapsFontNameforFontFamily:_fontFamily bold:self.boldTrait italic:self.italicTrait];
		}
		else
		{
			overrideName = [DODTCoreTextFontDescriptor overrideFontNameforFontFamily:_fontFamily bold:self.boldTrait italic:self.italicTrait];
		}
	}
...
- (BOOL)isEqual:(id)object
{
	...
	if (![object isKindOfClass:[DODTCoreTextFontDescriptor class]])
	{
		return NO;
	}
	
	DODTCoreTextFontDescriptor *otherFontDescriptor = object;
...
{
	DODTCoreTextFontDescriptor *newDesc = [[DODTCoreTextFontDescriptor allocWithZone:zone] initWithFontAttributes:[self fontAttributes]];
