...
@implementation DODTHTMLParserNode
{
	NSString *_name;
	DT_WEAK_VARIABLE DODTHTMLParserNode *_parentNode;
...
- (void)addChildNode:(DODTHTMLParserNode *)childNode
{
	@synchronized(self)
	{
...
- (void)removeChildNode:(DODTHTMLParserNode *)childNode
{
	@synchronized(self)
	{
...
- (void)_appendHTMLToString:(NSMutableString *)string indentLevel:(NSUInteger)indentLevel
{
	@synchronized(self)
	{
		...
		// output attributes
		for (DODTHTMLParserNode *childNode in _childNodes)
		{
...
- (NSString *)text
{
	@synchronized(self)
	{
		...
		for (DODTHTMLParserTextNode *oneChild in self.childNodes)
		{
			if ([oneChild isKindOfClass:[DODTHTMLParserTextNode class]])
			{
