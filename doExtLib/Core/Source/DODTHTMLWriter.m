...
- (NSString *)_tagRepresentationForListStyle:(DODTCSSListStyle *)listStyle closingTag:(BOOL)closingTag listPadding:(CGFloat)listPadding inlineStyles:(BOOL)inlineStyles
{
	BOOL isOrdered = NO;
...
- (void)_buildOutputAsHTMLFragment:(BOOL)fragment
{
	...
	for (NSUInteger i=0; i<[paragraphs count]; i++)
	{
		...
		DODTCSSListStyle *effectiveListStyle = [currentListStyles lastObject];
		
		// retrieve the paragraph style
		DODTCoreTextParagraphStyle *paragraphStyle = [paraAttributes paragraphStyle];
		...
		if (fontIsBlockLevel)
		{
			if (paragraphFont)
			{
				DODTCoreTextFontDescriptor *desc = [DODTCoreTextFontDescriptor fontDescriptorForCTFont:paragraphFont];
		...
		if ([previousListStyles count]>[currentListStyles count])
		{
			...
			do
			{
				DODTCSSListStyle *closingStyle = [closingStyles lastObject];
		...
		if (effectiveListStyle)
		{
			...
			else
			{
				...
				for (DODTCSSListStyle *oneList in currentListStyles)
				{
			...
			[listsToOpen enumerateObjectsUsingBlock:^(DODTCSSListStyle *oneList, NSUInteger idx, BOOL *stop) {
		...
		[_attributedString enumerateAttributesInRange:paragraphRange options:0 usingBlock:^(NSDictionary *attributes, NSRange spanRange, BOOL *stopEnumerateAttributes) {
			...
			if (!fontIsBlockLevel)
			{
				DODTCoreTextFontDescriptor *fontDescriptor = [attributes fontDescriptor];
			...
			DODTColor *textColor = [attributes foregroundColor];
			...
			DODTColor *backgroundColor = [attributes backgroundColor];
