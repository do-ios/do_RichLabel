...
#import "DODTAnchorHTMLElement.h"
...
@implementation DODTHTMLElement
{
...
+ (void)initialize
{
	...
	[tmpDict setObject:[DODTAnchorHTMLElement class] forKey:@"a"];
	[tmpDict setObject:[DODTBreakHTMLElement class] forKey:@"br"];
	[tmpDict setObject:[DODTHorizontalRuleHTMLElement class] forKey:@"hr"];
	[tmpDict setObject:[DODTListItemHTMLElement class] forKey:@"li"];
	[tmpDict setObject:[DODTStylesheetHTMLElement class] forKey:@"style"];
	[tmpDict setObject:[DODTTextAttachmentHTMLElement class] forKey:@"img"];
	[tmpDict setObject:[DODTTextAttachmentHTMLElement class] forKey:@"object"];
	[tmpDict setObject:[DODTTextAttachmentHTMLElement class] forKey:@"video"];
	[tmpDict setObject:[DODTTextAttachmentHTMLElement class] forKey:@"iframe"];
...
+ (instancetype)elementWithName:(NSString *)name attributes:(NSDictionary *)attributes options:(NSDictionary *)options
{
	...
	if (!class)
	{
		...
		Class attachmentClass = [DODTTextAttachment registeredClassForTagName:name];
		
		if (attachmentClass)
		{
			class = [DODTTextAttachmentHTMLElement class];
		}
		else
		{
			// use generic of none found
			class = [DODTHTMLElement class];
		}
	}
	
	DODTHTMLElement *element = [[class alloc] initWithName:name attributes:attributes options:options];
...
- (NSDictionary *)attributesForAttributedStringRepresentation
{
    ...
    if (_anchorId)
    {
        [tmpDict setObject:_anchorId forKey:DTAnchorAttributeId];
    }
...
- (BOOL)needsOutput
{
	@synchronized(self)
	{
		...
		{
			for (DODTHTMLElement *oneChild in self.childNodes)
			{
...
{
	DODTHTMLElement *parent = self.parentElement;
...
- (NSAttributedString *)attributedString
{
	@synchronized(self)
	{
		...
		else
		{
			...
			DODTHTMLElement *previousChild = nil;
		...
		if (self.displayStyle == DTHTMLElementDisplayStyleBlock && [tmpString length]>0)
		{
			...
			if ([self _isNotChildOfList])
			{
				...
				if (___useiOS6Attributes)
				{
					...
					DODTCoreTextParagraphStyle *paragraphStyle = [DODTCoreTextParagraphStyle paragraphStyleWithNSParagraphStyle:paraStyle];
...
#endif
				{
					...
					DODTCoreTextParagraphStyle *paragraphStyle = [DODTCoreTextParagraphStyle paragraphStyleWithCTParagraphStyle:paraStyle];
...
- (void)applyStyleDictionary:(NSDictionary *)styles
{
	...
	if (_displayStyle == DTHTMLElementDisplayStyleBlock)
	{
		...
		{
			// need a block
			DODTTextBlock *newBlock = [[DODTTextBlock alloc] init];
...
- (DODTCSSListStyle *)listStyle
{
	DODTCSSListStyle *style = [[DODTCSSListStyle alloc] initWithStyles:_styles];
...
- (void)inheritAttributesFromElement:(DODTHTMLElement *)element
{
    ...
    _anchorId = [element.anchorId copy];
...
- (void)setTextColor:(DODTColor *)textColor
{
...
- (void)setTextAttachment:(DODTTextAttachment *)textAttachment
{
...
- (DODTHTMLElement *)parentElement
{
	return (DODTHTMLElement *)self.parentNode;
}
