...
@interface DODTCoreTextLayoutLine ()

@property (nonatomic, strong) NSArray *glyphRuns;

@end

@implementation DODTCoreTextLayoutLine
{
	CGRect _frame;
	CTLineRef _line;
...
{
	NSInteger ret = 0;
	for (DODTCoreTextGlyphRun *oneRun in self.glyphRuns)
	{
...
- (DODTCoreTextLayoutLine *)justifiedLineWithFactor:(CGFloat)justificationFactor justificationWidth:(CGFloat)justificationWidth
{
	// make this line justified
	CTLineRef justifiedLine = CTLineCreateJustifiedLine(_line, justificationFactor, justificationWidth);
	
	DODTCoreTextLayoutLine *newLine = [[DODTCoreTextLayoutLine alloc] initWithLine:justifiedLine];
...
- (CGRect)frameOfGlyphsWithRange:(NSRange)range
{
	...
	for (DODTCoreTextGlyphRun *oneRun in glyphRuns)
	{
...
- (BOOL)isHorizontalRule
{
	...
	DODTCoreTextGlyphRun *singleRun = [runs lastObject];
		...
		if (!_glyphRuns)
		{
			...
			if (runCount)
			{
				...
				for (CFIndex i=0; i<runCount; i++)
				{
					...
					DODTCoreTextGlyphRun *glyphRun = [[DODTCoreTextGlyphRun alloc] initWithRun:oneRun layoutLine:self offset:position.x];
...
- (DODTCoreTextParagraphStyle *)paragraphStyle
{
	...
	DODTCoreTextGlyphRun *lastRun = [self.glyphRuns lastObject];
...
- (BOOL)writingDirectionIsRightToLeft
{
	if (_needsToDetectWritingDirection)
	{
		...
		{
			DODTCoreTextGlyphRun *firstRun = [self.glyphRuns objectAtIndex:0];
