...
@implementation DODTCoreTextGlyphRun
{
	CTRunRef _run;
	CGRect _frame;
	...
	DT_WEAK_VARIABLE DODTCoreTextLayoutLine *_line;	// retain cycle, since these objects are retained by the _line
	DT_WEAK_VARIABLE NSDictionary *_attributes; // weak because it is owned by _run IVAR
	NSArray *_stringIndices;
	
	DODTTextAttachment *_attachment;
	BOOL _hyperlink;
...
- (id)initWithRun:(CTRunRef)run layoutLine:(DODTCoreTextLayoutLine *)layoutLine offset:(CGFloat)offset
{
	self = [super init];
...
- (void)drawDecorationInContext:(CGContextRef)context
{
	...
	DODTColor *backgroundColor = [self.attributes backgroundColor];
