...
@implementation DODTCoreTextLayoutFrame
{
...
- (id)initWithFrame:(CGRect)frame layouter:(DODTCoreTextLayouter *)layouter range:(NSRange)range
{
	self = [super init];
...
- (id)initWithFrame:(CGRect)frame layouter:(DODTCoreTextLayouter *)layouter
{
...
- (CGPoint)_algorithmLegacy_BaselineOriginToPositionLine:(DODTCoreTextLayoutLine *)line afterLine:(DODTCoreTextLayoutLine *)previousLine
{
...
- (CGFloat)_algorithmWebKit_halfLeadingOfLine:(DODTCoreTextLayoutLine *)line
{
	CGFloat maxFontSize = [line lineHeight];
	
	DODTCoreTextParagraphStyle *paragraphStyle = [line paragraphStyle];
...
- (CGPoint)_algorithmWebKit_BaselineOriginToPositionLine:(DODTCoreTextLayoutLine *)line afterLine:(DODTCoreTextLayoutLine *)previousLine
{
	...
	if (previousLine)
	{
		...
		if ([self isLineLastInParagraph:previousLine])
		{
			DODTCoreTextParagraphStyle *paragraphStyle = [previousLine paragraphStyle];
	...
	for (DODTTextBlock *previousTextBlock in previousLine.textBlocks)
	{
	...
	for (DODTTextBlock *currentTextBlock in line.textBlocks)
	{
...
- (CGPoint)baselineOriginToPositionLine:(DODTCoreTextLayoutLine *)line afterLine:(DODTCoreTextLayoutLine *)previousLine options:(DTCoreTextLayoutFrameLinePositioningOptions)options
{
...
// deprecated
- (CGPoint)baselineOriginToPositionLine:(DODTCoreTextLayoutLine *)line afterLine:(DODTCoreTextLayoutLine *)previousLine
{
...
- (void)_buildLinesWithTypesetter
{
	...
	DODTCoreTextLayoutLine *previousLine = nil;
	...
	do  // for each line
	{
		...
		for (DODTTextBlock *oneTextBlock in textBlocks)
		{
		...
		// wrap it
		DODTCoreTextLayoutLine *newLine = [[DODTCoreTextLayoutLine alloc] initWithLine:line
	...
	{
		DODTCoreTextLayoutLine *lastLine = [_lines lastObject];
		
		CGFloat totalPadding = 0;
		
		for (DODTTextBlock *oneTextBlock in lastLine.textBlocks)
		{
...
- (void)_buildLinesWithStandardFramesetter
{
	...
	for (id oneLine in (__bridge NSArray *)cflines)
	{
		...
		DODTCoreTextLayoutLine *newLine = [[DODTCoreTextLayoutLine alloc] initWithLine:(__bridge CTLineRef)oneLine];
...
- (NSArray *)linesVisibleInRect:(CGRect)rect
{
	...
	for (DODTCoreTextLayoutLine *oneLine in self.lines)
	{
...
- (CGRect)_blockFrameForEffectiveRange:(NSRange)effectiveRange level:(NSUInteger)level
{
    ...
    DODTCoreTextLayoutLine *firstBlockLine = [self lineContainingIndex:effectiveRange.location];
    DODTCoreTextLayoutLine *lastBlockLine = [self lineContainingIndex:NSMaxRange(effectiveRange)-1];
    ...
    for (NSInteger i = [firstBlockLine.textBlocks count]-1; i>=level;i--)
    {
        ...
        DODTTextBlock *oneTextBlock = [firstBlockLine.textBlocks objectAtIndex:i];
    ...
    for (NSInteger i = [lastBlockLine.textBlocks count]-1; i>=level;i--)
    {
        ...
        DODTTextBlock *oneTextBlock = [lastBlockLine.textBlocks objectAtIndex:i];
    ...
    {
        DODTTextBlock *textBlock = [firstBlockLine.textBlocks objectAtIndex:i];
...
- (BOOL)_enumerateTextBlocksAtLevel:(NSUInteger)level inRange:(NSRange)range usingBlock:(void (^)(DODTTextBlock *textBlock, CGRect frame, NSRange effectiveRange, BOOL *stop))block
{
	...
	@synchronized((__bridge id)_framesetter)
	{
		...
		while (index<NSMaxRange(range))
		{
			...
			DODTTextBlock *blockAtLevelToHandle = [textBlocks objectAtIndex:level];
...
- (void)_enumerateTextBlocksInRange:(NSRange)range usingBlock:(void (^)(DODTTextBlock *textBlock, CGRect frame, NSRange effectiveRange, BOOL *stop))block
{
...
- (void)_drawTextBlock:(DODTTextBlock *)textBlock inContext:(CGContextRef)context frame:(CGRect)frame
{
...
- (void)_drawTextBlocksInContext:(CGContextRef)context inRange:(NSRange)range
{
	...
	[self _enumerateTextBlocksInRange:range usingBlock:^(DODTTextBlock *textBlock, CGRect frame, NSRange effectiveRange, BOOL *stop) {
...
{
	DODTColor *color = [dictionary objectForKey:@"Color"];
...
- (void)_drawHorizontalRuleFromLine:(DODTCoreTextLayoutLine *)line inContext:(CGContextRef)context
{
	// HR has only a single glyph run with a \n, but that has all the attributes
	DODTCoreTextGlyphRun *oneRun = [line.glyphRuns lastObject];
	...
	DODTColor *color = [oneRun.attributes foregroundColor];
	...
	DODTTextBlock *textBlock = [[oneRun.attributes objectForKey:DTTextBlocksAttribute] lastObject];
...
- (void)_setForgroundColorInContext:(CGContextRef)context forGlyphRun:(DODTCoreTextGlyphRun *)glyphRun options:(DTCoreTextLayoutFrameDrawingOptions)options
{
	DODTColor *color = nil;
...
- (void)drawInContext:(CGContextRef)context options:(DTCoreTextLayoutFrameDrawingOptions)options
{
	...
	for (DODTCoreTextLayoutLine *oneLine in visibleLines)
	{
		...
		for (DODTCoreTextGlyphRun *oneRun in oneLine.glyphRuns)
		{
			...
			DODTTextAttachment *attachment = oneRun.attachment;
...
- (CGRect)frame
{
	...
	if (_frame.size.width == CGFLOAT_WIDTH_UNKNOWN)
	{
		...
		for (DODTCoreTextLayoutLine *oneLine in _lines)
		{
...
- (CGRect)intrinsicContentFrame
{
	...
	DODTCoreTextLayoutLine *firstLine = [_lines objectAtIndex:0];
...
- (DODTCoreTextLayoutLine *)lineContainingIndex:(NSUInteger)index
{
	for (DODTCoreTextLayoutLine *oneLine in self.lines)
	{
...
- (BOOL)isLineFirstInParagraph:(DODTCoreTextLayoutLine *)line
{
...
- (BOOL)isLineLastInParagraph:(DODTCoreTextLayoutLine *)line
{
