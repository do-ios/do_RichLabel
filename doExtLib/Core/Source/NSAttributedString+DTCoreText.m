...
- (NSArray *)textAttachmentsWithPredicate:(NSPredicate *)predicate class:(Class)class
{
	...
	[self enumerateAttribute:NSAttachmentAttributeName inRange:entireRange options:NSAttributedStringEnumerationLongestEffectiveRangeNotRequired usingBlock:^(DODTTextAttachment *attachment, NSRange range, BOOL *stop) {
...
- (NSInteger)itemNumberInTextList:(DODTCSSListStyle *)list atIndex:(NSUInteger)location
{
	...
	// get outermost list
	DODTCSSListStyle *outermostList = [textListsAtIndex objectAtIndex:0];
    ...
    [string enumerateSubstringsInRange:range options:NSStringEnumerationByParagraphs usingBlock:^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop)
     {
		 ...
		 DODTCSSListStyle *currentEffectiveList = [textLists lastObject];
...
- (NSRange)rangeOfTextList:(DODTCSSListStyle *)list atIndex:(NSUInteger)location
{
...
- (NSRange)rangeOfTextBlock:(DODTTextBlock *)textBlock atIndex:(NSUInteger)location
{
...
{
	// create a writer
	DODTHTMLWriter *writer = [[DODTHTMLWriter alloc] initWithAttributedString:self];
...
+ (NSAttributedString *)prefixForListItemWithCounter:(NSUInteger)listCounter listStyle:(DODTCSSListStyle *)listStyle listIndent:(CGFloat)listIndent attributes:(NSDictionary *)attributes
{
	...
	DODTCoreTextFontDescriptor *fontDescriptor = nil;
	DODTCoreTextParagraphStyle *paragraphStyle = nil;
	
	if (paraStyle)
	{
		paragraphStyle = [DODTCoreTextParagraphStyle paragraphStyleWithCTParagraphStyle:paraStyle];
	...
	if (font)
	{
		fontDescriptor = [DODTCoreTextFontDescriptor fontDescriptorForCTFont:font];
	}
	...
	if (fontDescriptor)
	{
		...
		DODTCoreTextFontDescriptor *fontDesc = [fontDescriptor copy];
	...
	if (prefix)
	{
		...
		{
			image = [DODTImage imageNamed:listStyle.imageName];
